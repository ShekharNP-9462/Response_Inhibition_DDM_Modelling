    ...
    def callback(*args, **kwargs):
        ...
        if n % log_interval == 0 or n == total_steps:
            ...
            if n > 0:
                ...
                eta_str = "estimating..."
            print(f"  [BEESTS] steps {n}/{total_steps} | elapsed {elapsed / 60:.1f} min | {eta_str}")
    return callback


def main():
    parser = argparse.ArgumentParser(
        description="Fit single-subject BEESTS (ex-Gaussian race) to Rashi pre-cued stop-signal data.",
    )
    parser.add_argument(
        "subject_dir",
        type=str,
    ...
    print(f"[BEESTS] Started at {time.strftime('%Y-%m-%d %H:%M:%S')} (total steps: {total_steps})")
    ...
    print(f"[BEESTS] Finished at {time.strftime('%Y-%m-%d %H:%M:%S')} | total elapsed: {elapsed / 60:.1f} min")
    print(az.summary(trace, var_names=["mu_go", "sigma_go", "tau_go", "mu_ssrt", "sigma_ssrt", "tau_ssrt"]))

    if args.output:
        az.to_netcdf(trace, args.output)
        print(f"[BEESTS] Saved trace to {args.output}")
